		  ...
		  <table width="100%" border="0" cellpadding="0" cellspacing="10" class="main_tab">
			...
			<tr>
				<td class="font3 fftd">
					...
					薪水：<select  name="sal_id" style="width:100px;">
						   <option value="0">--薪水选择--</option>
						   <c:forEach items="${requestScope.sals }" var="sal">
						   		<c:choose>
			    					<c:when test="${employee.sal.id == sal.id }">
			    						<option value="${sal.id }" selected="selected">${sal.month}</option>
			    					</c:when>
			    					<c:otherwise>
			    						<option value="${sal.id }">${sal.month }</option>
			    					</c:otherwise>
			    				</c:choose>
			    			</c:forEach>
					</select>
